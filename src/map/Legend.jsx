// Legend — glass overlay listing visible layers with their symbology.
// Rendered inside <MapView>; collapses to a title bar.
...
export default function Legend() {
  ...
  const visibleLayers = useMemo(
    () => layers.filter((l) => l.visible !== false && l.geojson?.features?.length > 0),
    [layers]
  )
    ...
    <div className="legend-panel">
      ...
      {!collapsed && visibleLayers.map((layer) => {
        ...
        if (style.symbologyMode === 'categorical' && style.categoricalValues?.length) {
          return (
            <div key={layer.id}>
              <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', margin: '4px 0' }}>{layer.name}</div>
        ...
        return (
          <div key={layer.id} className="legend-item">
            ...
            <span style={{ fontSize: 9, marginLeft: 'auto', padding: '1px 5px', borderRadius: 4, background: 'rgba(0,212,200,0.10)', color: 'var(--accent-primary)' }}>
