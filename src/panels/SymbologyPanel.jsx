...
export default function SymbologyPanel() {
  const { layers, activeLayerId, updateLayerStyle, addToast } = useAppStore()
  ...
  // Sync tab when active layer changes
  useEffect(() => {
    setTab(style.symbologyMode || 'simple')
  }, [activeLayerId]) // eslint-disable-line
  ...
  if (!layer) {
    return (
      <FloatingPanel panelKey="symbology" title="Symbology" icon={<IconPalette size={16} />} defaultWidth={340} defaultHeight={520}>
        <div className="empty-state">
          <div className="empty-state-icon"><IconPalette size={32} /></div>
          <div className="empty-state-title">Select a layer</div>
        </div>
      </FloatingPanel>
    )
  }
  ...
  return (
    <FloatingPanel panelKey="symbology" title="Symbology" icon={<IconPalette size={16} />} defaultWidth={340} defaultHeight={580}>
      {/* Layer name */}
      ...
      {/* Mode tabs */}
      <div className="tabs" style={{ padding: '4px 12px', flexShrink: 0, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
